...
AUTH0_CLIENT_SECRET=$(op item get "$ONEPASSWORD_ITEM_NAME" --reveal --fields client_secret)
if [[ -z "$AUTH0_CLIENT_SECRET" ]]; then
    echo "Failed to retrieve client_secret from 1Password"
    exit 1
fi

CREDENTIALS=$(op item get "$ONEPASSWORD_ITEM_NAME" --reveal --fields client_secret,audience,domain,client_id)
...
TOKEN_RESPONSE=$(curl -s --request POST --url "https://${AUTH0_DOMAIN}/oauth/token" --header 'content-type: application/json' --data $REQUEST_BODY)
